                ...
                html.Div( [ dcc.Dropdown(
                    className="five columns",
                      ...
                      #  value='image1'
                          ...
                          html.Div([
                    html.Button(
                        children='Deduct cancer',
                         className="column",
                            n_clicks=0,
                            type='button',
                            style={'width': '25%' , 'margin-left': '20%'},
                            id='select-image'
                        )]
                    )])]),
            ##
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div
                            ...
                            html.Div(id=''),
                            html.Div(children='''
                            A web application framework for detect liver cancer by analyzing the CT scan.
                        '''),
...
@app.callback(
   Output('dd-output-container', 'children'),
    [Input('select-image', 'n_clicks')],
    [State('demo-dropdown', 'value'),
    State('p_name', 'value'),
    State('phone', 'value')])
def update_output(n_clicks,value, p_name, phone):
    predresult=1
    ...
    if value=='image1':
        ...
        f"""INSERT INTO Presults (name,phone ,user,result,predresult)
              VALUES('{p_name}','{phone}',{current_user.id},'assets/output.jpeg',{predresult});"""
          )
        conn.commit()
        return html.Img(src='assets/output.jpeg', style={'width': '100%', 'margin': '2%'})
    if value=='image2':
        time.sleep(20)
        conn = sqlite3.connect("patient.db")
        c = conn.cursor()
        ...
        c.execute(
        f"""INSERT INTO Presults (name,phone ,user,result,predresult)
              VALUES('{p_name}','{phone}',{current_user.id},'assets/output1.jpeg',{predresult});"""
          )
        conn.commit()        
        return html.Img(src='assets/output1.jpeg', style={'width': '100%', 'margin': '2%'})
    if value=='image3':
        time.sleep(20)
        conn = sqlite3.connect("patient.db")
        c = conn.cursor()
        ...
        c.execute(
        f"""INSERT INTO Presults (name,phone ,user,result,predresult)
              VALUES('{p_name}','{phone}',{current_user.id},'assets/output2.jpeg',{predresult});"""
          )
        conn.commit()
        return html.Img(src='assets/output2.jpeg', style={'width': '100%', 'margin': '2%'})
    else:
        return ''
